...
class EnvService {
  ...
  async updateEnvFile(updates) {
    ...
    const sanitizeValue = (key, value) => (redactedKeys.has(key) ? '[REDACTED]' : value);
    const safeUpdates = { ...updates };
    Object.keys(safeUpdates).forEach(key => {
      safeUpdates[key] = sanitizeValue(key, safeUpdates[key]);
    });
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        if (Object.prototype.hasOwnProperty.call(updates, key)) {
          ...
          this.logSettings(
            ` Aggiornamento ${key}: "${sanitizeValue(key, oldValue)}" -> "${sanitizeValue(key, newValue)}"`
          );
